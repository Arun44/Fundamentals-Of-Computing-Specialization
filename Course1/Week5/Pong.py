...
def draw(canvas):
    ...
    global paddle1_vel, paddle2_vel, BALL_RADIUS;

    # draw mid line and gutters
    canvas.draw_line([WIDTH / 2, 0], [WIDTH / 2, HEIGHT], 1, "purple")
    canvas.draw_line([PAD_WIDTH, 0], [PAD_WIDTH, HEIGHT], 1, "purple")
    canvas.draw_line([WIDTH - PAD_WIDTH, 0], [WIDTH - PAD_WIDTH, HEIGHT], 1, "purple")
    ...
    if ((paddle2_pos <= HEIGHT - PAD_HEIGHT and paddle2_vel > 0) or (paddle2_pos >= 0 and paddle2_vel < 0)):
        paddle2_pos = paddle2_pos + paddle2_vel;
    elif ((paddle1_pos <= HEIGHT - PAD_HEIGHT and paddle1_vel > 0) or (paddle1_pos >= 0 and paddle1_vel < 0)):
    ...
    # draw scores
    canvas.draw_text(str(score1), [210, 95], 65, "red")
    canvas.draw_text(str(score2), [320, 95], 65, "red")


def keydown(key):
    global paddle1_vel, paddle2_vel, padd_vel
    if (key == simplegui.KEY_MAP["w"]):
        paddle1_vel = -padd_vel
    elif (key == simplegui.KEY_MAP["s"]):
        paddle1_vel = padd_vel
    if (key == simplegui.KEY_MAP["up"]):
        paddle2_vel = -padd_vel
